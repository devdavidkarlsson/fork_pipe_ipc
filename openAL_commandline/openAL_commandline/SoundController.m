...
- (void)initAL {
  ...
  //[self playStream:@"bubbles" gain:1 pitch:0.1 loops:YES];
  //[self play];
...
-(void)preloadSources
{
  ...
  for (sourceIndex = 0; sourceIndex < sourceCount; sourceIndex++) {
    ...
    [sources addObject:[NSNumber numberWithUnsignedInt:(uint)sourceID]];
  }
}
...
-(NSUInteger)nextAvailableSource
{
  ...
  for (NSNumber * sourceNumber in sources) {
    alGetSourcei([sourceNumber unsignedIntValue], AL_SOURCE_STATE, (ALint*)&sourceState);
  ...
  NSInteger looping;
  for (NSNumber * sourceNumber in sources) {
    alGetSourcei([sourceNumber unsignedIntValue], AL_LOOPING, (ALint*)&looping);
    if (!looping) {
...
-(AudioFileID)openAudioFile:(NSString*)filename
{
  ...
  NSString *resourceFolder = [exePath stringByDeletingLastPathComponent];
  NSString *resourceFile = [resourceFolder stringByAppendingPathComponent:@"1.caf"];
  NSURL * afUrl = [NSURL fileURLWithPath:resourceFile];
...
#endif
  if (result != 0) NSLog(@"cannot openf file: %@",afUrl);
  return outAFID;
}
...
- (NSUInteger)playStream:(NSString*)soundKey gain:(ALfloat)gain pitch:(ALfloat)pitch loops:(BOOL)loops
{
  ...
  alSourcei((ALuint)sourceID, AL_BUFFER, 0);
  ...
  for (NSNumber * bufferNumber in bufferList) {
    ...
    alSourceQueueBuffers((ALuint)sourceID, 1, (ALuint*)&bufferID);
    err = alGetError();
...
-(BOOL)loadNextStreamingBufferForSound:(NSString*)key intoBuffer:(NSUInteger)bufferID
{
  ...
  UInt32 fileSize = (UInt32)([[record objectForKey:@"fileSize"] unsignedIntegerValue]);
  UInt32 bufferSize = (UInt32)([[record objectForKey:@"bufferSize"] unsignedIntegerValue]);
  UInt32 bufferIndex = (UInt32)([[record objectForKey:@"bufferIndex"] unsignedIntegerValue]);
  ...
  if (bufferIndex == totalChunks) {
    ...
    bufferSize = (UInt32)leftOverBytes;
  }
  ...
  alBufferData((ALuint)bufferID,format,outData,bytesToRead,freq);
  ...
  NSLog(@"BufferIndex: %d TotalChunks: %ld", bufferIndex, (long)totalChunks);
...
-(BOOL)rotateBufferForStreamingSound:(NSString*)soundKey
{
  ...
  alGetSourcei((ALuint)sourceID, AL_SOURCE_STATE, (ALint*)&sourceState);
  ...
  alGetSourcei((ALuint)sourceID, AL_BUFFERS_PROCESSED, (ALint*)&buffersProcessed);
  ...
  if (buffersProcessed > 0) {
    ...
    alSourceUnqueueBuffers((ALuint)sourceID, 1, (ALuint*)&bufferID);
